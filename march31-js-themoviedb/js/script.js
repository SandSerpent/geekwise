...
function getMovies(){
  'https://api.themoviedb.org/3/search/movie?api_key=<<api_key>>&language=en-US&query=godzilla&page=1&include_adult=false';
  var url = myVars.get_url() + '3/movie/550?api_key=' + myVars.get_api_key() + '&language=en-US&query=sleight';
...
function parseMovies(movies){
  ...
  ul = document.createElement('ul'), li poster_path, title, tagline, overview, homepage;
  ...
  movies.results.forEach(function(e, i){
    ...
    poster_path.src = 'http://image.tmdb.org/t/p/w500' + movie.poster_path;
